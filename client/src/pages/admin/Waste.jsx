...
function Waste() {
  ...
  const { data: apiData } = useFetch('/waste');

  const todayCost = mockReceivingWaste.filter((w) => w.date === '2026-04-08').reduce((s, w) => s + w.cost, 0)
    + mockAgingWaste.filter((w) => w.date === '2026-04-08').reduce((s, w) => s + w.cost, 0);
  const weekCost = [...mockReceivingWaste, ...mockAgingWaste].reduce((s, w) => s + w.cost, 0);
  ...
  return (
    ...
    >
      ...
      <motion.div variants={fadeInUp} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <KPICard title="Top Waste Product" value="Iceberg Lettuce" icon={AlertTriangle} />
      </motion.div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-4">
              <div>
                ...
                <Select>
                  ...
                  <SelectContent>
                    {['Roma Tomatoes', 'Cucumbers', 'Bell Peppers', 'Iceberg Lettuce', 'Avocados', 'Bananas'].map((p) => (
                ...
                {mockPendingAuth.map((entry) => (
                  <div key={entry.id} className="flex items-center justify-between p-3 bg-brand-warning/5 rounded-lg border border-brand-warning/20">
                    <div>
                      ...
                      <p className="text-brand-muted text-xs">{entry.type} waste: {entry.reason} - Submitted by {entry.submittedBy}</p>
                    </div>
                    <div className="flex gap-2">
                      <Button size="sm" variant="outline" className="text-brand-success border-brand-success/30">
      ...
      <motion.div variants={fadeInUp} className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          ...
          <CardContent>
            <DataTable columns={receivingColumns} data={mockReceivingWaste} />
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <DataTable columns={agingColumns} data={mockAgingWaste} />
          </CardContent>
        </Card>
      </motion.div>
