...
import { Building2, User, Mail, Phone, Lock, Check, ChevronRight, ChevronLeft, Eye, EyeOff, MapPin } from 'lucide-react';
    ...
    {steps.map((step, i) => (
      <div key={step.number} className="flex items-center">
        <div className="flex flex-col items-center">
          <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center border-2 transition-all duration-300 ${currentStep > step.number ? 'bg-brand-accent border-brand-accent' : currentStep === step.number ? 'border-brand-accent text-brand-accent' : 'border-white/20 text-white/60'}`}>
          ...
          <span className={`text-[10px] sm:text-xs mt-1.5 whitespace-nowrap ${currentStep >= step.number ? 'text-brand-accent' : 'text-white/60'}`}>{step.title}</span>
        </div>
        {i < steps.length - 1 && (
          <div className={`w-10 sm:w-16 h-0.5 mx-1.5 sm:mx-2 mb-5 sm:mb-6 transition-colors duration-300 ${currentStep > step.number ? 'bg-brand-accent' : 'bg-white/20'}`} />
        )}
      </div>
    ))}
  </div>
...
function Register() {
  ...
  const handleStep2 = step2Form.handleSubmit(async (data) => {
    ...
    try {
      ...
      const msg = resp?.error || resp?.message || (resp?.details?.[0]?.message) || 'Registration failed. Please try again.';
      toast.error(msg);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="min-h-screen relative bg-brand-base">
      ...
      <div className="fixed inset-0 z-0 overflow-hidden pointer-events-none mix-blend-screen">
        <div className="absolute w-[600px] h-[600px] rounded-full opacity-[0.08]" style={{ background: 'radial-gradient(circle, #4EECD3 0%, transparent 70%)', top: '-10%', right: '-10%', animation: 'mesh-move 20s ease-in-out infinite' }} />
        <div className="absolute w-[500px] h-[500px] rounded-full opacity-[0.05]" style={{ background: 'radial-gradient(circle, #B84EEC 0%, transparent 70%)', bottom: '-15%', left: '-5%', animation: 'mesh-move-reverse 25s ease-in-out infinite' }} />
      </div>
      ...
      <div className="relative z-10 min-h-screen flex flex-col lg:flex-row">
        ...
        <motion.div
          ...
          className="lg:hidden"
        >
          <div className="flex flex-col items-center pt-8 pb-6 px-6 drop-shadow-[0_2px_8px_rgba(0,0,0,0.7)]">
        ...
        <motion.div
          ...
          className="flex-1 flex items-start lg:items-center justify-center px-5 py-6 sm:px-8 sm:py-8 lg:p-12"
        >
            ...
            <AnimatePresence mode="wait" custom={currentStep}>
                  ...
                  <form onSubmit={handleStep1} className="space-y-3.5 sm:space-y-4">
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <Input {...step1Form.register('address')} placeholder="Street, city, country" className="pl-10 h-11" />
                      </div>
                      {step1Form.formState.errors.address && <p className="text-brand-error text-xs mt-1">{step1Form.formState.errors.address.message}</p>}
                    </div>
              ...
              {currentStep === 2 && (
                <motion.div key="step2" custom={1} variants={slideVariants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.3 }}>
                  <h2 className="text-xl sm:text-2xl font-bold text-brand-primary mb-1">Your Account</h2>
                  <p className="text-brand-secondary text-sm mb-5 sm:mb-8">Admin details and password for signing in</p>

                  <form onSubmit={handleStep2} className="space-y-4">
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <Label className="block mb-1.5 text-sm">First Name</Label>
                        <div className="relative">
                          <User className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-brand-muted" />
                          <Input {...step2Form.register('firstName')} placeholder="First name" className="pl-10 h-11" />
                        </div>
                        {step2Form.formState.errors.firstName && <p className="text-brand-error text-xs mt-1">{step2Form.formState.errors.firstName.message}</p>}
                      </div>
                      <div>
                        <Label className="block mb-1.5 text-sm">Last Name</Label>
                        <Input {...step2Form.register('lastName')} placeholder="Last name" className="h-11" />
                        {step2Form.formState.errors.lastName && <p className="text-brand-error text-xs mt-1">{step2Form.formState.errors.lastName.message}</p>}
                      </div>
                    </div>
